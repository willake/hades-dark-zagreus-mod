...
OnAnyLoad { "D_Boss01", function(triggerArgs)
    ...
    if ActiveEnemies ~= nil then
        ...
        for enemyId, enemy in pairs( ActiveEnemies ) do
            ...
            table.insert(enemy.WeaponOptions, "DarkRamaBow")
            EquipWeapon({ Name = "DarkRamaBow", DestinationId = enemy.ObjectId })
            table.insert(enemy.WeaponOptions, "DarkRamaBowDash")
            EquipWeapon({ Name = "DarkRamaBowDash", DestinationId = enemy.ObjectId })
            table.insert(enemy.WeaponOptions, "DarkRamaBowSplitShot")
            EquipWeapon({ Name = "DarkRamaBowSplitShot", DestinationId = enemy.ObjectId })
