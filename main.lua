...
OnAnyLoad { "D_Boss01", function(triggerArgs)
    ...
    if ActiveEnemies ~= nil then
        ...
        for enemyId, enemy in pairs( ActiveEnemies ) do
            ...
            table.insert(enemy.WeaponOptions, "DarkChironBow")
            EquipWeapon({ Name = "DarkChironBow", DestinationId = enemy.ObjectId })
            table.insert(enemy.WeaponOptions, "DarkChironBowDash")
            EquipWeapon({ Name = "DarkChironBowDash", DestinationId = enemy.ObjectId })
            table.insert(enemy.WeaponOptions, "DarkChironBowSplitShot")
            EquipWeapon({ Name = "DarkChironBowSplitShot", DestinationId = enemy.ObjectId })
