...
OnAnyLoad { "D_Boss01", function(triggerArgs)
    ...
    if ActiveEnemies ~= nil then
        ...
        for enemyId, enemy in pairs( ActiveEnemies ) do
            ...
            table.insert(enemy.WeaponOptions, "EnemySwordWeapon")
            EquipWeapon({ Name = "EnemySwordWeapon", DestinationId = enemy.ObjectId })
            -- UnequipWeapon({ Name = "EnemySwordWeapon", DestinationId = enemy.ObjectId })
            -- EquipWeapon({ Name = "EnemySwordWeapon", DestinationId = enemy.ObjectId })
            table.insert(enemy.WeaponOptions, "EnemySwordWeapon2")
            EquipWeapon({ Name = "EnemySwordWeapon2", DestinationId = enemy.ObjectId })
        end
    end
